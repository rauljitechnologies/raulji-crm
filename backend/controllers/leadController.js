  ...
  try {
    ...
    if (!name || !phone) {
      return res.status(400).json({
        success: false,
        error: { message: 'Name and phone required.' }
      });
    }

    if (email) {
      const exists = await prisma.lead.findFirst({
        where: { companyId, email, isDeleted: false }
      });
    ...
    await prisma.activity.create({
      data: {
        ...
        type: 'NOTE',
        description: `Lead created via ${source.toLowerCase()}`
      }
    });
...
exports.updateLead = async (req, res) => {
  try {
    ...
    const old = await prisma.lead.findFirst({
    ...
    const data = {
      ...
      ...(status && { status }),
      ...(priority && { priority }),
      ...(dealValue !== undefined && { dealValue: +dealValue }),
