...
exports.getLeads = async (req, res) => {
  try {
    ...
    const [leads, total] = await Promise.all([
      prisma.lead.findMany({ where, include:{ assignedTo:{ select:{ userId:true, name:true, avatar:true } } }, orderBy:{ [sortBy]:sortOrder }, skip:(page-1)*+limit, take:+limit }),
...
exports.updateLead = async (req, res) => {
  try {
    ...
    const lead = await prisma.lead.update({ where:{ leadId }, data:updateData });
...
exports.addActivity = async (req, res) => {
  try {
    ...
    const activity = await prisma.activity.create({ data:{ companyId, leadId, userId:req.user?.userId, type:type||'NOTE', description, metadata:metadata||null } });
